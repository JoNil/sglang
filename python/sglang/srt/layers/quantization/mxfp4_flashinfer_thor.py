...
class Mxfp4FlashinferThorMoEMethod:
    ...
    def _process_native_weights(
        ...
        *,
        ...
        if self._graph_max_tokens > 0:
            self._native_decode_workspace = allocate_native_mxfp4_workspace(
                num_experts=num_experts,
                top_k=int(self.moe_runner_config.top_k),
                hidden=hidden,
                intermediate=intermediate,
                max_tokens=self._graph_max_tokens,
                device=layer.w13_weight.device,
            )
    ...
    def apply(
        ...
        if (
            ...
            workspace = (
                self._native_decode_workspace
                if self._native_decode_workspace is not None
                and num_tokens <= self._graph_max_tokens
                else get_cached_native_mxfp4_workspace(
                    num_experts=int(layer.num_local_experts),
