...
__global__ void compute_sm100_cutlass_group_gemm_args(
    ElementA* A, ElementB* B, ElementSFA* SFA, ElementSFB* SFB, ElementD* D, int* m_indptr,
    int* expert_ids, int n, int k, int num_groups, ProblemShape* problem_sizes, const ElementA** A_ptr,
  ...
  int weight_group = expert_ids == nullptr ? i : expert_ids[i];
  ...
  if constexpr (SwapAB) {
    ...
    A_ptr[i] = safe_inc_ptr(A, int64_t(weight_group) * int64_t(n) * int64_t(k));
    ...
    SFA_ptr[i] =
        safe_inc_ptr(SFA, int64_t(weight_group) * int64_t(sf_n) * int64_t(sf_k));
    ...
    B_ptr[i] = safe_inc_ptr(B, int64_t(weight_group) * int64_t(n) * int64_t(k));
    ...
    SFB_ptr[i] =
        safe_inc_ptr(SFB, int64_t(weight_group) * int64_t(sf_n) * int64_t(sf_k));
  }
}
...
cudaError_t CutlassMXFP4GroupwiseScaledGroupGEMMSM100(
  ...
  if constexpr (SwapAB) {
    FLASHINFER_CUDA_CALL(cudaLaunchKernelEx(&config, prepare_args_kernel, B, A, SFB, SFA, D,
                                            m_indptr, expert_ids, n, k, num_groups, problem_sizes,
                                            A_ptr, B_ptr, SFA_ptr, SFB_ptr, D_ptr, stride_A,
                                            stride_B, stride_D, layout_SFA, layout_SFB));
  } else {
