...
void CutlassGroupGemmMXFP4GroupwiseScaledSM100Impl(
    TensorView int_workspace_buffer, TensorView float_workspace_buffer, TensorView A, TensorView B,
    TensorView SFA, TensorView SFB, TensorView D, TensorView m_indptr, int* expert_ids, int64_t n,
    int64_t k, int64_t mma_sm, int64_t tile_m, int64_t tile_n, int64_t tile_k, bool swap_ab) {
                ...
                return DISPATCH_SWAP_AB(swap_ab, SWAP_AB, [&] {
                  if constexpr (is_valid_config<c_type_in_a, c_type_in_b, c_type_sf_a, c_type_sf_b,
                    ...
                    auto status = flashinfer::group_gemm::CutlassMXFP4GroupwiseScaledGroupGEMMSM100<
                        ...
                        static_cast<int*>(m_indptr.data_ptr()), expert_ids, n, k, num_groups,
                        stream);
...
void CutlassGroupGemmMXFP4GroupwiseScaledSM100(TensorView int_workspace_buffer,
  ...
  CutlassGroupGemmMXFP4GroupwiseScaledSM100Impl(
      int_workspace_buffer, float_workspace_buffer, A, B, SFA, SFB, D, m_indptr, nullptr, n, k,
      mma_sm, tile_m, tile_n, tile_k, swap_ab);
}

void CutlassGroupGemmMXFP4GroupwiseScaledIndexedSM100(
    TensorView int_workspace_buffer, TensorView float_workspace_buffer, TensorView A, TensorView B,
    TensorView SFA, TensorView SFB, TensorView D, TensorView m_indptr, TensorView expert_ids,
    int64_t n, int64_t k, int64_t mma_sm, int64_t tile_m, int64_t tile_n, int64_t tile_k,
    bool swap_ab) {
  CutlassGroupGemmMXFP4GroupwiseScaledSM100Impl(
      int_workspace_buffer, float_workspace_buffer, A, B, SFA, SFB, D, m_indptr,
      static_cast<int*>(expert_ids.data_ptr()), n, k, mma_sm, tile_m, tile_n, tile_k, swap_ab);
}
